...
const getCursorXY = (input, selectionPoint) => {
  ...
  document.getElementsByClassName('App')[0].children[0].appendChild(div)
  // get the marker position, this is the caret position top and left relative to the input
  const { offsetLeft: spanX, offsetTop: spanY } = span
  // lastly, remove that dummy element
  // NOTE:: can comment this out for debugging purposes if you want to see where that span is rendered
  document.getElementsByClassName('App')[0].children[0].removeChild(div)
...
function updateBoundingBox(suggestionText){
  ...
  if (cursorPosition < textArea.textContent.trim().length){
    Y_OFFSET = -16;
    suggestionBox.classList.add('mid')
    if(suggestionBox.classList.contains('end')){
      suggestionBox.classList.remove('end')
    }
  } else {
   suggestionBox.classList.add('end')
    if(suggestionBox.classList.contains('mid')){
      suggestionBox.classList.remove('mid')
    } 
  }
  ...
  let suggestionTextDisplayArray = suggestionText.split(" ")
  
  if(suggestionTextDisplayArray[0] != ''){
   suggestionTextDisplayArray[0] = `<span id="first">${suggestionTextDisplayArray[0]}</span>` 
  }
  let displayText=suggestionTextDisplayArray[0] + `<span id="interpunct">·</span>`;
  for(let i=1; i<suggestionTextDisplayArray.length-1; i++){
    displayText = displayText + suggestionTextDisplayArray[i] + `<span id="interpunct">·</span>` 
  }
  displayText += suggestionTextDisplayArray[suggestionTextDisplayArray.length-1];
  suggestionBox.innerHTML = displayText
...
export default function Editor() {
  ...
  return (
    <div>
      ...
      <button className="submitButton" onClick={submit}>Submit review</button>
